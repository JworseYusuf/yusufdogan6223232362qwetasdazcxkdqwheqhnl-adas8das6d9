...
exports.run = (client, message, args) => {
  ...
  if (reason.length < 1) return message.reply('Sunucudan Neden Yasaklamak İstediğini Söylermisin.');
  if (message.mentions.users.size < 1) return message.reply('Kimi banlayacağını yazmalısın.').catch(console.error);

  if (!message.guild.member(user).bannable) return message.reply('Yetkim Yetmiyor Lütfen Tekrar Deneyiniz..');
...
exports.help = {
  ...
  usage: 'd?yasakla [kullanıcı] [sebep]'
};
