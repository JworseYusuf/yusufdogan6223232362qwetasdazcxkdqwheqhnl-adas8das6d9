...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("» HayDo BOT | Destek Sistemi")
        .setDescription("**Buradaki destek ekibimiz sizinle ilgilenecektir.\nDestek talebini iptal etmek için [dve!talepkapat](https://discord.gg/U2byS7x) yazabilirsin!**")
        .setFooter(' HayDo BOT | Destek Sistemi', client.user.avatarURL)
        .setTimestamp()
        ch.send(embed).catch()
        ch.send("Sayın : @!   HayDoMC#3548")
        ch.send("@here")
        ch.awaitMessages((msg)=> {
            if (msg.content === "?!talepkapat") {
