...
client.on('message', async msg => {
  if (msg.content.toLowerCase() === 'sa') {
    await msg.react('🇦');
    msg.react('🇸');
    msg.reply('**Aleyküm Selam Birader , İşşallah İyisindir =)** .')
  }
  });
...
client.elevation = message => {
  ...
  if (message.author.id === '488402676116226058') permlvl = 4;
  return permlvl;
};
...
client.on("guildMemberAdd", async member => {
  ...
  const skanal31 = member.guild.channels.find('name', skanal9)
    skanal31.send(`**:inbox_tray: ${member.user.tag}** Katıldı **${sayac}** olmamıza son **${sayac - member.guild.members.size}** üye kaldı :white_check_mark: `)
});
...
client.on("guildMemberRemove", async member => {
  ...
  if (!skanal9) return;
  const skanal31 = member.guild.channels.find('name', skanal9)
    skanal31.send(`**:outbox_tray: ${member.user.tag}** Ayrıldı **${sayac}** olmamıza son **${sayac - member.guild.members.size}** üye kaldı :x: `)
});
